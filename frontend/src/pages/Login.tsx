...
export default function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Mock validation - allowing any email for testing purposes
      if (email.includes('@') && password.length >= 3) {
        dispatch(login({
          name: email.split('@')[0].replace(/[^a-zA-Z0-9]/g, ' ').toUpperCase(),
          email,
        ...
        setError('Unauthorized biometrics. Valid email and password required.');
        setLoading(false);
      }
    }, 1500);
  };
    ...
    <div className="min-h-screen bg-[#05070A] text-white flex items-center justify-center relative overflow-hidden font-sans">
      ...
      >
        <div className="text-center mb-10 border-b border-white/5 pb-8">
          ...
          >
            <span className="font-extrabold text-3xl tracking-tighter mix-blend-overlay">Ae</span>
          </motion.div>
        ...
        <p className="text-center mt-8 text-sm text-[#8B949E]">
          Unregistered Bio-signature? <Link to="/signup" className="text-[#00F2FF] hover:text-white transition-colors underline underline-offset-4 pointer">Create Protocol</Link>
        </p>
      </motion.div>
    </div>
  );
