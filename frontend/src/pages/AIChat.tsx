...
import { Bot, User, Send, ShieldAlert, Sparkles, Loader2, Mic, Image as ImageIcon, Volume2, X, Square } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-[#05070A] p-2 md:p-6 pb-20 md:pb-6 relative overflow-hidden">
      ...
      >
        ...
        <div>
          ...
          <p className="text-white/50 text-sm">Vision, Voice, Disease suggestions & Precautions</p>
        </div>
      </motion.div>
      ...
      <div className="mb-4 bg-[#FF4D4D]/10 border border-[#FF4D4D]/20 rounded-xl p-3 flex gap-3 items-start relative z-10">
        ...
        <p className="text-xs text-[#FF4D4D]/90">
          This AI is for informational purposes only. Do not use this as a replacement for actual medical diagnosis or professional treatment. Always consult a qualified healthcare provider.
        </p>
      </div>
            ...
            >
              <div className={`flex gap-3 max-w-[85%] md:max-w-[70%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-1 ${
                  msg.sender === 'user' ? 'bg-[#00F2FF]/20 text-[#00F2FF]' : 'bg-[#FF0055]/20 text-[#FF0055]'
                }`}>
                ...
                <div className={`p-4 rounded-2xl flex flex-col gap-2 ${
                  ...
                  {msg.image && (
                    <img src={msg.image} alt="User upload" className="max-w-full h-auto rounded-lg mb-2 max-h-48 object-cover border border-white/10" />
                  )}
        ...
        <div className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-3 w-full">
          ...
          {selectedImage && (
            <div className="relative inline-block w-max">
              <img src={selectedImage} alt="Preview" className="h-20 w-auto rounded-md border border-white/20 object-cover" />
              <button 
          ...
          <div className="flex gap-2 w-full items-end">
            ...
            <input 
              ...
              placeholder={isListening ? "Listening... Speak now!" : "Type symptoms or upload an image..."}
              className={`flex-1 bg-black/40 border rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none transition-all text-sm ${
                isListening ? 'border-[#FF0055]/50 shadow-[0_0_15px_rgba(255,0,85,0.2)]' : 'border-white/10 focus:border-[#00F2FF]/50 focus:ring-1 focus:ring-[#00F2FF]/50'
              }`}
            />
