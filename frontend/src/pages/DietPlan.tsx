...
import { Activity, Flame, Moon, Search, AlertTriangle, ChevronRight, Loader2, Sparkles } from 'lucide-react';
...
export default function DietPlan() {
  ...
  const lifestyle = useSelector((state: RootState) => state.lifestyle); // Get simulation parameters

  const [isGenerating, setIsGenerating] = useState(false);
  const [aiStrategy, setAiStrategy] = useState<string | null>(null);
  const [aiMeals, setAiMeals] = useState<any[] | null>(null);
    ...
    try {
      const res = await fetch('http://localhost:8000/api/v1/generate-diet', {
        ...
        body: JSON.stringify({
          lifestyle,
          profile,
          target_calories: Math.floor((Math.floor(10 * (Number(profile.weight) || 70) + 6.25 * (Number(profile.height) || 170) - 5 * (Number(profile.age) || 30) + 5)) * (lifestyle.steps_daily < 5000 ? 1.2 : lifestyle.steps_daily < 8000 ? 1.375 : lifestyle.steps_daily < 12000 ? 1.55 : 1.725))
        })
      });
  ...
  const hasMedical = profile.conditions.length > 0 && !profile.conditions.includes('None');
  const dietStrategy = useMemo(() => {
    let strategy = "Optimized balanced macros to sustain daily activity and vitality.";
    if (hasDiabetes) strategy = "Low GI focused regimen to stabilize insulin spikes. Focus on high fiber.";
    else if (hasHypertension) strategy = "DASH-aligned diet low in sodium. Emphasize potassium-rich foods.";
    else if (hasThyroid) strategy = "Moderate cruciferous intake. Support with iodine & selenium.";

    if (lifestyle.steps_daily < 4000) {
      strategy += " Sedentary lifestyle detected: Caloric deficit via strict portion control is necessary.";
    } else if (lifestyle.steps_daily > 10000) {
      strategy += " High kinetic output detected: Increased complex caloric load needed for optimal muscle recovery.";
    }
    return strategy;
  ...
  const meals = useMemo(() => {
    ...
    if (lifestyle.steps_daily < 4000) {
      baseMeals[1].desc = "Strictly reduce carbs to half portion since activity is very low. " + baseMeals[1].desc;
      ...
      baseMeals[1].desc += " Add an extra Carb source like a Sweet Potato to fuel your high steps!";
      baseMeals[1].macros = 'High Energy Carbs';
    } else if (lifestyle.steps_daily >= 15000) {
      baseMeals[1].desc += " Extreme activity level! Double your complex carbs and add an afternoon Banana.";
    ...
    if (lifestyle.stress_level <= 3) {
      ...
      baseMeals[2].desc += " Include Omega-3s (Walnuts/Flaxseeds) to maintain stable cortisol.";
    } else if (lifestyle.stress_level > 6) {
      baseMeals[2].desc += " HIGH STRESS DETECTED! Must add Magnesium-rich foods: Spinach, Pumpkin Seeds, or dark chocolate to lower cortisol spikes.";
    }

    if (lifestyle.hydration_liters < 2.0) {
      baseMeals[0].desc = "CRITICAL: Dehydration detected. Drink 2-3 massive glasses of water first! " + baseMeals[0].desc;
    } else if (lifestyle.hydration_liters >= 2.0 && lifestyle.hydration_liters <= 3.5) {
      baseMeals[0].desc = "Start with a glass of lukewarm water. " + baseMeals[0].desc;
    } else if (lifestyle.hydration_liters > 3.5) {
      baseMeals[0].desc = "Excellent hydration status. Kidneys flushing toxins optimally. " + baseMeals[0].desc;
    }

    return baseMeals;
  }, [hasMedical, lifestyle.steps_daily, lifestyle.stress_level, lifestyle.hydration_liters]);
    ...
    <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      {hasMedical && (
        ...
        >
          ...
          <div>
            <h3 className="text-sm font-bold text-status-danger uppercase tracking-wider mb-1">Health Warning Active</h3>
            <p className="text-sm text-white/80">User profile indicates medical predispositions: {profile.conditions.filter(c => c !== 'None').join(', ')}. Routine strictly adjusted.</p>
            {profile.medications && (
                <p className="text-xs text-white/60 mt-1 italic">Contraindications checked for medications: {profile.medications}</p>
            )}
          </div>
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative z-10">
        ...
        <motion.div className="lg:col-span-1 space-y-6">
           <div className="glassmorphism p-6 rounded-2xl border-t border-t-[#00F2FF]/30 h-full flex flex-col justify-between">
              <div>
                <h4 className="text-xs text-text-muted font-bold tracking-widest uppercase mb-4">Vitality Bridge</h4>
                <div className="flex justify-between items-center mb-6">
                   <div className="text-center">
                     <p className="text-[#8B949E] text-xs font-mono mb-1">CURRENT</p>
                     <div className="text-3xl font-light text-white">{currentScore ? currentScore.toFixed(1) : '--'}</div>
                   </div>
                   <motion.div 
                     animate={{ x: [0, 5, 0] }}
                   ...
                   <div className="text-center">
                     <p className="text-[#7000FF] font-bold text-xs font-mono mb-1 pulse-text">TARGET</p>
                     <div className="text-3xl font-light text-[#00F2FF] drop-shadow-[0_0_10px_rgba(0,242,255,0.3)]">{targetScore ? targetScore.toFixed(1) : '--'}</div>
                   </div>
                </div>
                
                <div className="bg-black/30 p-4 rounded-xl border border-white/5 space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-white/60 flex items-center gap-2"><Activity className="w-4 h-4 text-[#00F2FF]" /> Optimization Gap</span>
                    <span className="font-mono text-[#00F2FF]">+{gap > 0 ? gap.toFixed(1) : '0.0'} pts</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-white/60 flex items-center gap-2"><Flame className="w-4 h-4 text-[#FF4D4D]" /> Daily Calc Calories</span>
                    <span className="font-mono">{targetCalories} kcal</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-white/60 flex items-center gap-2"><Search className="w-4 h-4 text-[#7000FF]" /> Protocol</span>
                    <span className="text-xs text-[#7000FF] bg-[#7000FF]/10 px-2 py-0.5 rounded font-mono">ACTIVE</span>
                  </div>
                </div>
              </div>
               ...
               <div className="mt-6 pt-4 border-t border-white/5">
                 ...
                 <p className="text-sm text-white/70 leading-relaxed font-light">{sleepSuggestion}</p>
               </div>
           </div>
        </motion.div>
               ...
               <div className="flex justify-between items-start mb-6">
                  <div>
                    <h4 className="text-xs text-[#7000FF] font-bold tracking-widest uppercase mb-1 flex items-center gap-2">
                       ...
                       <button onClick={handleGenerateAI} disabled={isGenerating} className="ml-2 bg-[#7000FF]/20 hover:bg-[#7000FF]/40 text-[#00F2FF] px-2 py-1 rounded text-[10px] flex items-center gap-1 transition-colors">
                         {isGenerating ? <Loader2 className="w-3 h-3 animate-spin"/> : <Sparkles className="w-3 h-3"/>}
                         {isGenerating ? "GENERATING..." : "AI GENERATE"}
                       </button>
                    </h4>
                    <p className="text-sm text-white/60">{aiStrategy || dietStrategy}</p>
                  </div>
                 <div className="hidden md:block text-right">
                   <p className="text-xs tracking-widest font-mono text-text-muted">DAILY INTAKE</p>
                   <p className="text-xl font-light text-[#00F2FF]">{targetCalories} <span className="text-sm text-white/50">kcal</span></p>
                 </div>
              </div>

              <div className="space-y-4">
                {(aiMeals || meals).map((meal: any, idx: number) => (
                  <motion.div 
                    key={idx}
                  ...
                  >
                     <div>
                       <h5 className="font-bold text-[#00F2FF] text-sm tracking-wider mb-1">{meal.name}</h5>
                       <p className="text-white/80 text-sm leading-relaxed">{meal.desc}</p>
                     </div>
