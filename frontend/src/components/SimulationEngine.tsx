...
const SimulationEngine: React.FC = () => {
  ...
  return (
    <div className="p-6 glassmorphism rounded-2xl w-full">
      ...
      {renderSlider('Daily Steps', lifestyle.steps_daily, 'steps_daily', 0, 25000, 500, <Activity size={16}/>, 'steps')}
      {renderSlider('Sleep Duration', lifestyle.sleep_hours, 'sleep_hours', 0, 12, 0.5, <Moon size={16}/>, 'hrs')}
      {renderSlider('Hydration', lifestyle.hydration_liters, 'hydration_liters', 0, 8, 0.5, <Droplets size={16}/>, 'L')}
      {renderSlider('Stress Level', lifestyle.stress_level, 'stress_level', 1, 10, 1, <Activity size={16}/>, '/10')}
      
      <p className="text-xs text-text-muted mt-4 text-center">
        Adjust parameters to simulate future trajectory based on current health vectors.
      </p>
    </div>
  );
};
