...
export default function Progress() {
  ...
  const currentScore = prediction.current_score
    ? Number(parseFloat(prediction.current_score.toString()).toFixed(1))
    : 0;
  ...
  const aiSummary = useMemo(() => {
    if (logs.length < 2)
      return "Insufficient telemetry data. Please log your progress daily to generate AI insights.";
    ...
    if (latestLog.lifestyle.steps_daily > firstLog.lifestyle.steps_daily) {
      summary +=
        "Your increased kinetic output (steps) is significantly driving your longevity trajectory upward. ";
    }
    if (
      latestLog.lifestyle.sleep_hours >= 7 &&
      latestLog.lifestyle.sleep_hours > firstLog.lifestyle.sleep_hours
    ) {
      summary +=
        "Improved circadian consistency has fundamentally optimized your cellular repair metrics. ";
    } else if (
      latestLog.lifestyle.stress_level < firstLog.lifestyle.stress_level
    ) {
      summary +=
        "A measured reduction in cortisol (stress) levels is actively preventing bio-degradation.";
    }
    return summary;
  }, [logs]);
      ...
      {logs.length === 0 ? (
        <div className="glassmorphism p-12 text-center rounded-2xl border border-white/10">
          ...
          <p className="text-text-muted max-w-md mx-auto">
            Navigate to your Dashboard and select "Save Daily Progress" to
            initialize your health telemetry tracking.
          </p>
        </div>
      ) : (
        <motion.div
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <motion.div
              ...
              className="glassmorphism p-6 rounded-2xl border-t border-t-[#8B949E]/30 relative overflow-hidden group"
            >
            ...
            <motion.div
              variants={itemVariants}
              className="glassmorphism p-6 rounded-2xl border-t border-t-[#39FF14]/30 relative overflow-hidden group shadow-[0_0_20px_rgba(57,255,20,0.05)]"
            >
            ...
            <motion.div
              variants={itemVariants}
              className="glassmorphism p-6 rounded-2xl border-t border-t-[#7000FF]/30 relative overflow-hidden group shadow-[0_0_20px_rgba(112,0,255,0.05)]"
            >
              ...
              <div className="text-4xl font-light text-white font-mono tracking-tighter drop-shadow-[0_0_10px_rgba(112,0,255,0.3)]">
                ...
                <span className="text-xl text-[#8B949E]">Pts</span>
              </div>
              {gap > 0 && (
                <p className="text-xs text-white/70 mt-3 leading-relaxed">
                  ...
                  away from peak potential. {gapAdvice}
                </p>
              )}
            </motion.div>
            ...
            <motion.div
              variants={itemVariants}
              className="glassmorphism p-6 rounded-2xl border-t border-t-[#FF0055]/30 relative overflow-hidden group shadow-[0_0_20px_rgba(255,0,85,0.05)]"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-[#FF0055]/5 to-transparent pointer-events-none" />
              <h4 className="text-xs text-[#FF0055] font-bold tracking-widest uppercase mb-4">
                Body Metrics
              </h4>
              <div className="flex flex-col justify-end h-full mt-2">
                <div className="text-4xl font-light text-white font-mono tracking-tighter drop-shadow-[0_0_10px_rgba(255,0,85,0.3)]">
                  {bmi > 0 ? bmi.toFixed(1) : "--"}{" "}
                  <span className="text-xl text-[#8B949E]">BMI</span>
                </div>
          ...
          <motion.div
            variants={itemVariants}
            className="glassmorphism p-6 rounded-2xl border border-white/5 space-y-6 bg-black/40"
          >
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">
                Historical Trend
              </h3>
              <span className="text-xs text-[#00F2FF] font-mono bg-[#00F2FF]/10 px-2 py-1 rounded">
                VITALITY INDEX
              </span>
            </div>
            ...
            <div className="p-4 bg-[#7000FF]/10 border border-[#7000FF]/20 rounded-xl flex items-start gap-4">
              ...
              <p className="text-sm text-[#00F2FF] leading-relaxed">
                <span className="font-bold uppercase tracking-wider text-[#7000FF]">
                  Neural Summary:{" "}
                </span>
                {aiSummary}
              </p>
              ...
              {[...logs].reverse().map((log, idx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/50 font-mono mt-1">
                        Steps: {log.lifestyle.steps_daily} • Sleep:{" "}
                        {log.lifestyle.sleep_hours}hr
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-[#00F2FF] uppercase tracking-widest">
                      Score
                    </p>
                  </div>
                </motion.div>
