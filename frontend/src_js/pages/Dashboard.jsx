...
export default function Dashboard() {
  ...
  const handleSaveProgress = () => {
    ...
    const newLog = {
      ...
      optimized: Number(
        parseFloat(prediction.optimized_score.toString()).toFixed(1),
      ),
      lifestyle: { ...lifestyle },
    };
    ...
    localStorage.setItem(
      `aeterna_progress_logs${emailKey}`,
    ...
    <div className="p-4 md:p-8 relative">
      ...
      >
        <motion.header
          ...
          className="flex flex-col md:flex-row justify-between items-center pb-6 border-b border-white/5"
        >
          <div>
          ...
          <div className="flex items-center gap-6 mt-4 md:mt-0">
            ...
            >
              ...
              {hasLoggedToday
                ? "Reality Logged"
                : saveStatus || "Log Today's Reality"}
            </button>
            ...
            <div className="hidden md:flex flex-col items-end text-xs font-mono text-text-muted">
              ...
              <span className="text-[#8B949E]/70">
                UPLINK V2.2.0 <span className="text-[#00F2FF]">ACTIVE</span>
              </span>
            </div>
          </div>
        </motion.header>
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          >
            <div className="flex flex-col items-center justify-center w-full">
              <LongevityClock
                score={prediction.optimized_score || prediction.current_score}
              />
              <p className="text-[10px] text-[#00F2FF]/70 mt-2 text-center font-mono uppercase tracking-widest bg-[#00F2FF]/5 px-3 py-1 rounded-full border border-[#00F2FF]/20 shadow-[0_0_10px_rgba(0,242,255,0.1)]">
                Calibrated for {profile.age}yo {profile.gender} with{" "}
            ...
            >
              ...
              <div className="text-3xl font-light text-white mb-1 tracking-tight">
                ...
                <span className="text-lg text-text-muted">kcal</span>
              </div>
              <p className="text-xs text-text-muted/80 leading-relaxed">
                Your Daily Calorie Maintenance Target based on BMR (
                {bmr ? bmr.toLocaleString() : "--"} kcal) and Activity Level.
              </p>
            </motion.div>
          </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                <h4 className="text-xs text-text-muted font-bold tracking-widest uppercase mb-2">
                  Base Trajectory
                </h4>
                <div className="text-4xl font-light text-white mb-2 tracking-tight">
                  {prediction.loading && !prediction.current_score ? (
                    ...
                    parseFloat(prediction.current_score.toString()).toFixed(1)
                  ) : (
                    "--"
                  )}
                </div>
                <p className="text-sm text-text-muted/80 leading-relaxed">
                  Projected median vitality score under current unoptimized
                  bio-metrics mapped across 60 months.
                </p>
              </motion.div>
              ...
              >
                ...
                <h4 className="text-xs text-[#7000FF] font-bold tracking-widest uppercase mb-2 flex justify-between items-center">
                  Simulation Cap
                  <span className="px-2 py-0.5 rounded bg-[#7000FF]/20 text-[10px] text-[#00F2FF]">
                    OPTIMIZED
                  </span>
                </h4>
                ...
                <p className="text-sm text-[#00F2FF]/70 leading-relaxed">
                  Maximum theoretical index achievable by strictly adhering to
                  ML-suggested lifestyle corrections.
                </p>
              </motion.div>
            </div>
          </motion.div>
